...
for benchmark in bcast gather scatter reduce allreduce scan; do

	echo $benchmark
	mpirun -np $RANKS --hostfile ../hostfile --map-by node ./build/fmi_benchmark ${benchmark} > res/${benchmark}_${RANKS}.txt
